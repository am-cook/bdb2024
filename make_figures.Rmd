...
```{r}
HOLDOUT_keep_preds_df <- read.csv(paste0(SETTINGS$AGGREGATED_DATA, 'HOLDOUT_keep_preds_df.csv'))
mean_keep_forcing <- mean(HOLDOUT_keep_preds_df$forcing)
mean_keep_no_forcing <- mean(HOLDOUT_keep_preds_df$no_forcing)


HOLDOUT_handoff_preds_df <- read.csv(paste0(SETTINGS$AGGREGATED_DATA, 'HOLDOUT_handoff_preds_df.csv'))
mean_handoff_forcing <- mean(HOLDOUT_handoff_preds_df$forcing)
mean_handoff_no_forcing <- mean(HOLDOUT_handoff_preds_df$no_forcing)
```

```{r}
melted_holdout_keep_preds_df <- reshape2::melt(HOLDOUT_keep_preds_df, measure.vars = c('no_forcing', 'forcing'),
                                     variable.name = 'forcing_or_not',
                                     value.name = 'predicted_yds')
melted_holdout_handoff_preds_df <- reshape2::melt(HOLDOUT_handoff_preds_df, measure.vars = c('no_forcing', 'forcing'),
                                     variable.name = 'forcing_or_not',
                                     value.name = 'predicted_yds')
```
...
```{r}
holdout_keep_density <- ggplot(melted_holdout_keep_preds_df, aes(x = predicted_yds, color = forcing_or_not)) +
  geom_density() +
  geom_vline(xintercept = mean_keep_forcing, color = 'red', linetype = 'dashed') +
  geom_vline(xintercept = mean_keep_no_forcing, color = 'darkblue', linetype = 'dashed') +
  theme_classic() +
  scale_color_manual(values = c('no_forcing' = 'red', 'forcing' = 'darkblue')) +
  theme(legend.position = 'none')

ggsave(plot = holdout_keep_density, filename = paste0(SETTINGS$OUTPUT_FIGS, 'holdout_keep_density.png'))

holdout_keep_density
```

```{r}
holdout_handoff_density <- ggplot(melted_holdout_handoff_preds_df, aes(x = predicted_yds)) +
  geom_density(aes(color = forcing_or_not)) +
  geom_vline(xintercept = mean_handoff_forcing, color = 'red', linetype = 'dashed') +
  geom_vline(xintercept = mean_handoff_no_forcing, color = 'darkblue', linetype = 'dashed') +
  theme_classic() + 
  scale_color_manual(values = c('no_forcing' = 'red', 'forcing' = 'darkblue'))+ 
  theme(legend.position = 'none')
...
```{r}
forced_densities <- ggarrange(holdout_keep_density, holdout_handoff_density, ncol = 2, nrow = 1)
